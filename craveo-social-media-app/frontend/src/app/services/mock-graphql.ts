 ...
 * This provides mock data for development before backend is ready
 * Replace with actual GraphQL queries when backend is available
 */
...
export const mockGraphQL = {
  ...
  queries: {
    getCurrentUser: async (): Promise<User> => {
      await delay(500);
      return mockUsers[2]; // Return "me" user
    },
  ...
  mutations: {
    ...
    followUser: async (userId: string): Promise<{ isFollowing: boolean; followers: number }> => {
      ...
      // Toggle follow state
      const newFollowingState = !user.followers; // Simplified logic
      const newFollowers = newFollowingState ? user.followers + 1 : Math.max(0, user.followers - 1);
...
// Hook for using mock GraphQL in development
